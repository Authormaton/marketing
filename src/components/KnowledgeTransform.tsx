 ...
 *
 * @param {KnowledgeTransformProps} props - Component props
 * @param {string} [props.title] - Optional custom title to display. Defaults to "Transforming Knowledge into Intelligence"
 */
interface KnowledgeTransformProps {
  title?: string;
}

const KnowledgeTransform: React.FC<KnowledgeTransformProps> = ({ title }) => {
  const displayTitle = title ?? "Transforming Knowledge into Intelligence";
  return (
